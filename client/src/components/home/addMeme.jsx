...
const AddMeme = ({ profileAndAddMemeRef }) => {
    ...
    try {
      if (preview) {
        const action = await dispatch(
          addMemeAction({ media: preview, status })
        );
        if (!action.payload.error) {
          setStatus("");
          setPreview("");
          dispatch(restartMemeError());
          profileAndAddMemeRef.current.classList.remove("showForm");
        }
      }
    } catch (error) {
      console.log(error);
    ...
    <section className="addForm">
      <form onSubmit={postMeme}>
        ...
        <textarea
          value={status}
          onChange={auto_grow}
          placeholder="What's your meme"></textarea>
        <input
          value={preview}
          type="text"
          placeholder="img/gif link"
        ...
        <div className="addMemeBtns">
          ...
          <input
            type="submit"
            disabled={(preview ? false : true) || pending}
            value={
              pending
                ? "uploading..."
                : !preview
                ? "submit (must have an img/gif)"
                : "submit"
            }
          />
        </div>
